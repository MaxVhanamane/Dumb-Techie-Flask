...
def home():
    ...
    last = math.ceil(len(posts) / int(params["no-of-posts"]))  # Here we are taking greatest integer
    # pagination logic
    page = request.args.get('page')
    if (not str(page).isnumeric()):  # if page_num is not numeric then make it 1
        page = 1
    page=int(page)
    posts = posts[(page - 1) * int(params["no-of-posts"]): (page - 1) * int(params["no-of-posts"]) + int(
        params["no-of-posts"])]
...
def edit(srno):
    if 'user' in session and session['user'] == params["admin_user"]:
        if request.method == 'POST':
            ...
            if srno == '0':
                post = Posts(title=box_title, tagline=tagline, slug=slug, img_file=img_file, content=content, date=date)
                db.session.add(post)
                db.session.commit()
